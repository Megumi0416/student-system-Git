    ...
    <el-dialog title="学生信息" v-model="data.formVisible" width="40%" destroy-on-close>
      <el-form ref="form" :model="data.form" :rules="rules" label-width="70px" style="padding: 20px">
        ...
        <el-form-item prop="sex" label="性别">
          <el-input v-model="data.form.sex" placeholder="请输入性别"></el-input>
        </el-form-item>
<!--        <el-form-item prop="birth" label="生日">-->
<!--          <el-input v-model="data.form.birth" placeholder="请输入生日"></el-input>-->
<!--        </el-form-item>-->
        <el-form-item prop="grade" label="年级">
          <el-input v-model="data.form.grade" placeholder="请输入年级"></el-input>
        </el-form-item>
...
const del = (id) => {
  ElMessageBox.confirm('删除后数据无法恢复，您确定删除吗?', '删除确认', { type: 'warning' }).then(res => {
    if (res === 'confirm') { // 通常确认按钮的响应值为 'confirm'
      request.delete('/student/delete/' + id).then(response => {
...
const delBatch = () => {
  ...
  ElMessageBox.confirm('删除后数据无法恢复，您确定删除吗?', '删除确认', { type: 'warning' }).then(res => {
    request.delete('/student/delete/batch', { data: data.ids } ).then(response => {
      if (response.code === '200') {
